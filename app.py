...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 15 13:50:47 2019
...
def index():
    ...
    if user_query:
        ...
        # Parse the command
            ...
            qw = user_query    # Actual query word(s)
            ge = ''
        ...
        text=list(raw['text'])
        #cleaning for un-required items
        for i in range(len(text)):
            text[i]=text[i].lower()
            text[i]=text[i].replace('#','')
            text[i]=re.sub("\\\\\w\w","",text[i])
            text[i]=re.sub("b'rt @\w+: ","",text[i])
            text[i]=re.sub("&\w+","",text[i])
            text[i]=re.sub('b"rt @\w+: ',"",text[i])
            text[i]=re.sub("@\w+","",text[i])
            text[i]=re.sub("https://\w*\d*\D*\w*","",text[i])
        
        from textblob import TextBlob
        newdb=pd.DataFrame({'text':text})
        subjectivity=list(map(lambda x: TextBlob(x).subjectivity,newdb['text']))
        polarity=list(map(lambda x: TextBlob(x).polarity,newdb['text']))
        newdb['sub']=subjectivity
        newdb['pol']=polarity
        newdb['flag']=0
...
#        import plotly.tools as plotly_tools
        ...
        py.iplot(fig2,filename="subjectivity")
        ...
        newdb['flag'] = newdb.apply(lambda row: flag(row['pol']), axis=1)
        pos_tweets=newdb[newdb['flag']==1]
        pos_tweets=pos_tweets['text']
        neg_tweets=newdb[newdb['flag']==0]
        ...
        positive=wordcloud_draw(pos_tweets, "white")
        WordCloud.to_file(positive, filename="static/pos.png")
        negative=wordcloud_draw(neg_tweets, "black")
        WordCloud.to_file(negative, filename="static/neg.png")
    #-----------------------------------------------------
    #rendering the HTML files for the webapp
        return render_template('index.html', var=user_query)
    else:
